...
export default function Menu() {
    ...
    // Garante que a mesa mude para ocupada ao acessar o cardápio
    useEffect(() => {
    ...
    const fetchMenuData = async () => {
        try {
            ...
            const { data: catData } = await supabase.from('categorias').select('*').eq('empresa_id', empresaId).eq('ativo', true).order('ordem');
            setCategorias(catData || []);

            const { data: prodData } = await supabase.from('produtos').select('*').eq('empresa_id', empresaId).eq('ativo', true).order('nome');
    ...
    const fetchMeusPedidos = async (cmdId: string) => {
        const { data } = await supabase.from('pedidos').select('*').eq('comanda_id', cmdId).order('created_at', { ascending: false });
    ...
    return (
        <div className="min-h-screen bg-background pb-24">
            <header className="sticky top-0 z-50 bg-primary text-primary-foreground p-4 shadow-lg flex justify-between items-center">
                <div>
