...
export default function Caixa() {
  ...
  const handlePrint = () => {
    ...
    const itens = selectedComanda.pedidos?.map((p: any) => ({
    ...
    printCaixaReceipt({
      ...
      couver: includeCouver && couverAtivo ? { quantidade: couverQuantidade, valorUnitario: couverValorConfig, total: couverTotal } : undefined,
      total,
      formaPagamento: pagamentoMultiplo ? 'multiplo' : (formaPagamento || undefined),
      troco: trocoPara ? Math.max(0, parseFloat(trocoPara) - total) : undefined,
      timestamp: new Date(),
    });
