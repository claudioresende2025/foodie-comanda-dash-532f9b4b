-- ============================================================================
-- EXECUTAR NO SUPABASE: Setup automático de mesas e cardápio demo
-- Data: 2026-02-25
-- ============================================================================
-- Este script deve ser executado no SQL Editor do Supabase
-- Ele cria o trigger que automaticamente gera mesas e cardápio demo
-- para TODOS os novos restaurantes que se cadastrarem
-- ============================================================================
...
-- ============================================================================
-- OPCIONAL: Aplicar para empresas existentes que não têm mesas
-- Descomente e execute se quiser criar dados demo em empresas existentes
-- ============================================================================

/*
-- Para aplicar em TODAS as empresas que não têm mesas ainda:
DO $$
DECLARE
  v_empresa RECORD;
  v_count integer := 0;
BEGIN
  FOR v_empresa IN 
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM public.mesas m WHERE m.empresa_id = e.id
    )
  LOOP
    RAISE NOTICE 'Criando dados iniciais para: %', v_empresa.nome_fantasia;
    PERFORM public.setup_dados_iniciais_empresa(v_empresa.id);
    v_count := v_count + 1;
  END LOOP;
  
  RAISE NOTICE 'Total de empresas processadas: %', v_count;
END;
$$;
*/

-- ============================================================================
-- PARA APLICAR EM UMA EMPRESA ESPECÍFICA (por nome):
-- ============================================================================

/*
-- Exemplo: Aplicar para "Restaurante Taz Mania"
DO $$
DECLARE
  v_empresa_id uuid;
BEGIN
  ...
  WHERE nome_fantasia ILIKE '%Taz Mania%'
  LIMIT 1;
  
  IF v_empresa_id IS NOT NULL THEN
    PERFORM public.setup_dados_iniciais_empresa(v_empresa_id);
...
WHERE event_object_table = 'empresas'
  AND trigger_name = 'trigger_criar_dados_iniciais_empresa';
